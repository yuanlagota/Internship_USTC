...
!=======================================================================
  ...
  use flare_mmesh_parameters, only: n_interpolate, npXqo
  use flare_mmesh_inner_boundary, only: inner_boundary
  class(t_element)                :: this
  type(t_mfs_mesh), intent(inout) :: M
  integer,          intent(in)    :: irside, ipside, iblock
  class(qfunc),     intent(in)    :: Sr, Sp
  ...
  if (M%ir0 < 0) then
     ...
     call M%setup_boundary_nodes(irside, RADIAL, radial_interface(iri)%C, debug=debug)
  endif
  ! quasi-orhogonal mesh in upstream divertor legs
  np0 = (this%np - this%T%nt) / 10;   if (npXqo > 0) np0 = npXqo
  np1  = 0
  npA_range = 0
...
!     endif
     call M%make_divertor_grid(UPPER, this%np-np0, npA_range, Sp, U, Sr, this%T, ir0, ierr)
     if (ierr /= 0) then
...
!     endif
     call M%make_divertor_grid(LOWER, np0, npA_range, Sp, U, Sr, this%T, ir0, ierr)
     if (ierr /= 0) then
        write (6, 9000);  write (6, 9001) ierr;  stop
     endif
  ...
  ! 3. intermediate domain
     ...
     if (this%map_r(LOWER) == CORE) then
        if (n_interpolate+2 > M%nr-1) call ERROR("n_interpolate must not exceed nr-2")
        call M%make_orthogonal_grid(rrange=(/2+n_interpolate, M%nr-1/), addX=addX)
        call M%make_interpolated_mesh(2+n_interpolate, Sr, &
           inner_boundary(0,iblock)%hypersurf2d, inner_boundary(1,iblock)%hypersurf2d, &
           inner_boundary(1,iblock)%max_psiN)
